...
const columns: Record<SectionKey, Column[]> = {
  events: [
    ...
    { key: "event_type", label: "Format", kind: "eventType" },
    { key: "organizer_name", label: "Organisateur" },
    { key: "organizer_email", label: "E-mail", kind: "email" },
  ...
  dates: [
    { key: "event_title", label: "Sortie" },
    { key: "position", label: "Ordre" },
    { key: "starts_at", label: "Départ / date", kind: "date" },
    { key: "ends_at", label: "Retour", kind: "date" },
...
function rowKey(section: SectionKey, row: DataRow) {
  if (section === "votes") return `${row.participant_id}:${row.date_option_id}`;
  if (section === "stageVotes") return `${row.participant_id}:${row.place_id}`;
...
export default function AdminPage() {
  ...
  async function deleteRow(row: DataRow) {
    ...
    const warning = active === "events"
      ? `Supprimer définitivement la sortie « ${label} » et toutes les données associées ?`
      : `Supprimer définitivement cette ligne (${label}) ?`;
    if (!window.confirm(warning)) return;
    setDeleting(selectedRowKey);
    setError("");
    setNotice("");
    try {
      const response = await fetch("/api/admin/delete", {
        ...
        body: JSON.stringify({
          ...
          relatedId: active === "votes" ? row.date_option_id : row.place_id,
        }),
      });
      const payload = await response.json() as { error?: string };
      if (!response.ok) throw new Error(payload.error || "Suppression impossible.");
      setNotice("Ligne supprimée dans Supabase.");
    ...
    <main className="admin-shell">
      ...
      <section className="workspace">
        ...
        <section className="table-card">
          ...
          {(error || notice) && <div className={`table-message ${error ? "error" : "success"}`}>{error || notice}</div>}
              ...
              <tbody>
                {rows.map((row, index) => (
                  <tr key={String(row.id || `${active}-${index}`)}>
                    ...
                    <td><button className="delete-row" type="button" disabled={deleting === rowKey(active, row)} onClick={() => void deleteRow(row)}>{deleting === rowKey(active, row) ? "Suppression…" : "Supprimer"}</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
